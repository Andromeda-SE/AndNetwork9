...
public class RegisterCandidateRequestConsumerStrategy : IBrokerConsumerWithResponseStrategy<CandidateRequest, int>
{
    ...
    {
        Abstractions.Models.Member member;
        Abstractions.Models.Member[] members = await _coreDataContext.Members.Where(x =>
        ...
        switch (members.Length)
        {
            case 0:
            {
                EntityEntry<Abstractions.Models.Member> memberEntry = await _coreDataContext.Members.AddAsync(new()
                {
