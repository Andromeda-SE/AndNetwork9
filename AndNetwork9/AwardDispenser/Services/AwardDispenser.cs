...
public class AwardDispenser : ITimerService
{
    ...
    public async Task Process()
    {
        ...
        Member[] members = data.Members
            .Where(x => x.SteamId.HasValue && x.Rank > Rank.None && x.Rank < Rank.Advisor).ToArray();
        List<PlayerActivityResultNode> nodes = new();
        for (int i = 0; i < members.Length; i += 100)
            nodes.AddRange(
                await _playerActivitySender.CallAsync(members.Skip(i).Take(i + 100).Select(x => x.SteamId!.Value)
