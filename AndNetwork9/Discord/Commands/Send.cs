...
public class Send : Base
{
    ...
    public async Task Online(string message)
    {
        await ReplyAsync((await SendMessages(message,
                    Data.Members.Where(x => x.DiscordId.HasValue && x.Rank > Rank.None).AsAsyncEnumerable().Where(
                        x =>
                            Bot.GetUser(x.DiscordId.Value).Status is > UserStatus.Offline and < UserStatus
                                .DoNotDisturb))
                .ConfigureAwait(false))
            .ToString()).ConfigureAwait(false);
    }
    ...
    {
        if (member.DiscordId is null) return false;
        try
        {
            await Bot.GetUser(member.DiscordId.Value).SendMessageAsync(text).ConfigureAwait(false);
