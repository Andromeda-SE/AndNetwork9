...
public class TaskController : ControllerBase
{
    ...
    [Authorize]
    public async System.Threading.Tasks.Task<ActionResult<Task[]>> GetMeAssigneeAnalysis()
    {
        Member? member = await this.GetCurrentMember(_data).ConfigureAwait(false);
        if (member is null) return Unauthorized();
        ...
        return Ok(_data.Tasks.Where(x =>
            ...
            && (x.Status == TaskStatus.New 
                || x.Status == TaskStatus.ToDo)).OrderBy(x => x));
    }

    [HttpGet("me/assignee/active")]
    [Authorize]
    public async System.Threading.Tasks.Task<ActionResult<Task[]>> GetMeAssigneeActive()
    {
        Member? member = await this.GetCurrentMember(_data).ConfigureAwait(false);
        if (member is null) return Unauthorized();

        return Ok(_data.Tasks.Where(x => x.AssigneeId == member.Id 
                                         && (x.Status == TaskStatus.ToDo 
                                             || x.Status == TaskStatus.InProgress)).OrderBy(x => x));
    }


    [HttpGet("me/assignee/closed")]
    [Authorize]
    public async System.Threading.Tasks.Task<ActionResult<Task[]>> GetMeAssigneeClosed()
    {
        Member? member = await this.GetCurrentMember(_data).ConfigureAwait(false);
        if (member is null) return Unauthorized();
        ...
        return Ok(_data.Tasks.Where(x => x.AssigneeId == member.Id 
                                         && (x.Status <= TaskStatus.Canceled 
                                             || x.Status >= TaskStatus.Resolved)).OrderBy(x => x));
    }
